...
int read(const char *Path, struct StudentScore StudentData[])
{
...
int main()
{
    ...
    int length = read("score.txt", StudentData);
    struct StudentAverageScore SAVData[5] = {};
    ave(StudentData, SAVData, length);
    sort(SAVData, length);
    output("score_avg.txt", SAVData, length);
    return 0;
}